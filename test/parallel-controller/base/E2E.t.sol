...
import { BasePostDeployTestsBase } from "./PostDeployTests.t.sol";
 ...
 *         Base fork as the deployer, asserts the same end state the post-deploy test
 *         checks, then simulates the governance spell and pushes a real CCTP V2 burn through
 *         the new controller to prove the Base -> Ethereum route works.
 *
 ...
 */
abstract contract BaseParallelE2ETestsBase is BasePostDeployTestsBase {
    ...
    uint32 internal BASE_CCTP_DOMAIN     = 6;

    Bridge internal bridge;
    Domain internal mainnet;
    Domain internal base;
    ...
    function test_removeIntegrationIsKillSwitch() external {
        ...
        deal(usdc, address(almProxy), 2e6);
...
contract BaseParallelE2ETestLocal is BaseParallelE2ETestsBase {
    ...
    /// @dev Mirrors DeploySparkPAUParallelBase.run() + DeploySparkPAUParallelBase._deployFacets().
    function _runDeployScript() internal {
        ...
        controller        = IControllerFull(pauFactory.deployController(address(accessControls), Base.ALM_PROXY, address(rateLimits)));
        administeredAgent = IAdministeredAgent(agentFactory.deploy(deployer));
    }

    /// @dev Mirrors ConfigureSparkPAUParallelBase.run() with BaseStaging overrides.
...
contract BaseParallelE2ETestLive is BaseParallelE2ETestsBase {
